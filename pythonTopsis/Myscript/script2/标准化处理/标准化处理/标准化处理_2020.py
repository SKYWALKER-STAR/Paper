...
def sumOfStdResult(filePath:str):
    ...
    for i in range(0,len(df)):
        sum += df.loc[i,"result_std_2020"]
    
    df.loc[19,"result_std_2020"] = sum;
    df.to_excel(filePath)


def computePij(filePath:str):
    df = pd.read_excel(filePath)
    sumOfBij = df.loc[19,"result_std_2020"]

    sumOfPij = 0
    for i in range(0,len(df)):
        singleBij = df.loc[i,"result_std_2020"]
        Pij = (singleBij/sumOfBij);

        df.loc[i,"P(ij)_2020"] = Pij;

        if Pij != 0:
            df.loc[i,"LnP(ij)_2020"] = np.log(Pij)
        sumOfPij += df.loc[i,"P(ij)_2020"]

    df.loc[20,"P(ij)_2020"] = sumOfPij
...
def computeTheFuckingEntropy(pathroot:str,sheet_list:list):
    ...
    for j in range(0,15):
        ...
        for i in company_list:
            value_pij = i.loc[j,"P(ij)_2020"]
            value_lnpij = i.loc[j,"LnP(ij)_2020"]
    ...
    for i in range(0,len(company_list)):
        ...
        for j in range(0,len(list_of_index_with_ln)):
            company_list[i].loc[j,"entroy_2020"] = list_of_index_with_ln[j]
    ...
    for i in range(0,len(company_list)):
        ...
        for j in range(0,len(list_of_weight)):
            company_list[i].loc[j,"W(ij)_2020"] = list_of_weight[j]
        company_list[i].to_excel(pathfile,index=False)
...
def transformUnits(filePath:str):
    df = pd.read_excel(filePath)
    for i in range(0,15):
        value = df.loc[i,"value_2020"]
        if value > 1000:
            value = value / 100000000
            df.loc[i,"value_2020"] = value
...
def modifycation(filePath:str):
    ...
    df.loc[16,"value_2020"] = max_value;
    df.loc[17,"value_2020"] = min_value;
    df.loc[18,"value_2020"] = maxMinusmin
    df.to_excel(filePath,index=False)
...
def doTheInsert(pathroot:str,sheet_list:list):
    for i in sheet_list:
       ...
       ds.insert(loc=4,column="result_std_2020",value=0)
       ds.insert(loc=5,column="P(ij)_2020",value=0)
       ds.insert(loc=6,column="LnP(ij)_2020",value=0)
       ds.insert(loc=7,column="entropy_2020",value=0)
       ds.insert(loc=8,column="W(ij)_2020",value=0)
...
def dealAllDocsModify(pathroot:str,sheet_list:list):
    for i in sheet_list:
        ...
        dataStanderlization(filePath,"value_2020","result_std_2020")
        modifyNo(filePath)
